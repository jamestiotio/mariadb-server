...
static int online_alter_log_init(void *p)
{
  ...
  online_alter_hton->rollback= [](handlerton *hton, THD *thd, bool all) -> int
  {
    int res= online_alter_end_trans(get_cache_list(hton, thd), thd,
  ...
  online_alter_hton->commit_by_xid= [](handlerton *hton, XID *xid) -> int
  {
  ...
  online_alter_hton->rollback_by_xid= [](handlerton *hton, XID *xid) -> int
  {
    int res= online_alter_end_trans(*xid->online_alter_cache, current_thd,
                                    true, false);
    delete xid->online_alter_cache;
