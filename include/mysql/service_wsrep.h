...
extern struct wsrep_service_st {
  ...
  long long                   (*wsrep_xid_seqno_func)(const struct XID *xid);
  const unsigned char*        (*wsrep_xid_uuid_func)(const struct XID *xid);
...
extern "C" long long wsrep_xid_seqno(const struct XID* xid);
const unsigned char* wsrep_xid_uuid(const struct XID* xid);
